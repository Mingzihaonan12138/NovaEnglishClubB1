...
export function StarMascot({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 259.66 249.7" className={className} role="img" aria-label="Nova English Club mascot">
      <path
        fill={GOLD}
...
export default function Mascot({
  ...
  return (
    <div className={`flex flex-col items-center justify-center relative select-none ${className}`}>
      ...
      >
        <StarMascot className="w-full h-auto drop-shadow-sm pointer-events-none" />
      </motion.div>
    </div>
  );
}
